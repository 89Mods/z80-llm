...
const uint32_t crc32c_table[256*8] = {
...
uint32_t crc32c_sw(uint32_t crc, const uint8_t *data, uint32_t len) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        crc = crc32c_table[7*256 + (word & 0xff)] ^
              crc32c_table[6*256 + ((word >> 8) & 0xff)] ^
              crc32c_table[5*256 + ((word >> 16) & 0xff)] ^
              crc32c_table[4*256 + ((word >> 24) & 0xff)] ^
              crc32c_table[3*256 + ((word >> 32) & 0xff)] ^
              crc32c_table[2*256 + ((word >> 40) & 0xff)] ^
              crc32c_table[1*256 + ((word >> 48) & 0xff)] ^
              crc32c_table[0*256 + (word >> 56)];
    }
    data += n << 3;
    len &= 7;
    while (len) {
